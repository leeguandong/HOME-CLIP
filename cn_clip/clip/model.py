...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor): # inference:: 1,3,224,224
        x = self.conv1(x)  # shape = [*, width, grid, grid] 1,768,14,14
        ...
        x = x + self.positional_embedding.to(x.dtype) # 1,197,768
        x = self.ln_pre(x) # 1,197,768

        x = x.permute(1, 0, 2)  # NLD -> LND 197,1,768
        x = self.transformer(x) # 197,1,768
