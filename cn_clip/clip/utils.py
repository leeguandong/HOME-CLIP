...
def load_from_name(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu",
    ...
    if name in _MODELS:
        ...
        model_path = Path(os.path.dirname(__file__)).parent.parent / f"weights/{_MODELS[name]}"
    else:
        raise RuntimeError(f"Model {name} not found; available models = {available_models()}")

    # model_path = r'E:\common_tools\Chinese-CLIP-master\clip_cn_vit-b-16.pt'
    # model_path = Path(os.path.dirname(__file__)).parent.parent / "weights/clip_cn_vit-b-16.pt"
...
def create_model(model_name, checkpoint=None):
    ...
    vision_model_config_file = Path(
        os.path.dirname(__file__)).parent.parent / f"configs/{vision_model.replace('/', '-')}.json"
    ...
    text_model_config_file = Path(
        os.path.dirname(__file__)).parent.parent / f"configs/{text_model.replace('/','-')}.json"
