...
def main():
    ...
    vision_model_config_file = Path(
        os.path.dirname(__file__)).parent.parent / f"configs/{args.vision_model.replace('/', '-')}.json"
    ...
    text_model_config_file = Path(
        os.path.dirname(__file__)).parent.parent / f"configs/{args.text_model.replace('/','-')}.json"
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.should_save and num_steps_this_epoch > 0:
            if (epoch + 1) == args.max_epochs or (
                ...
                logging.info("Saved checkpoint {} (epoch {} @ {} steps) (writing took {} seconds)".format(save_path, epoch + 1, steps,
                                                                                                          time.time() - t1))
            ...
            logging.info(
                "Saved checkpoint {} (epoch {} @ {} steps) (writing took {} seconds)".format(save_path, epoch + 1, steps, time.time() - t1))
